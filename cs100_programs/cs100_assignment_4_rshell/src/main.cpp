...
const string OUT_ONE_STRING = ">";
const string OUT_TWO_STRING = ">>";
		...
		if (foundPrecedence != std::string::npos || (foundPrecedence != std::string::npos && foundTest != std::string::npos) || (foundPrecedence != std::string::npos && (foundOutput != std::string::npos && foundInput != std::string::npos && foundPipe != std::string::npos))) {
            ...
            while(p!=0) {
				...
				if (checkingPrecedenceF != NULL && (checkingInput != NULL || checkingPipe != NULL || checkingOutput != NULL)) {
					...
					string pString = superTree->SepCommands(p);
					totalString += pString;
                ...
                else if(checkingPrecedenceF != NULL && checkingPrecedenceF != NULL && checkingTestB != NULL && checkingTestE != NULL) {
                    while(checkingPrecedenceF != NULL) {
                        totalString += "( ";
                        superTree->splitUpFirstCharacter(p);
                        checkingPrecedenceF = (char *) memchr(p, '(', strlen(p));
            ...
            if(connectors.size() > 0) {
				...
				else if (temp == IN_STRING) {
					In *n = new In(lhs, rhs);
					completedListToRun.push(n);
				}
				else if (temp == OUT_ONE_STRING) {
					OutOne *n = new OutOne(lhs, rhs);
					completedListToRun.push(n);
				}
				else if (temp == OUT_TWO_STRING) {
					OutTwo *n = new OutTwo(lhs, rhs);
					completedListToRun.push(n);
				}
				else if (temp == PIPE_STRING) {
                ...
                while(connectors.size() > 0) {
					...
					else if (temp2 == IN_STRING) {
						In *n = new In(tempLHS, rhs);
						completedListToRun.push(n);
					}
					else if (temp2 == OUT_ONE_STRING) {
						OutOne *n = new OutOne(tempLHS, rhs);
						completedListToRun.push(n);
					}
					else if (temp2 == OUT_TWO_STRING) {
						OutTwo *n = new OutTwo(tempLHS, rhs);
						completedListToRun.push(n);
					}
					else if (temp2 == PIPE_STRING) {
