...
//	int i = 0; //iteration i
//	int j = 0; //iteration j
//	int first = 0; //first in container
//	int size = container->size(); //size of container
//	for( i = size - 1; i >= 0; --i ) { //basic iteration from the right to left
//		first = 0; // making sure first = 0, in this for loop
//		for( j = 1; j <= 1; ++j ) { //checking each value ...
//			if(container->at(j)->evaluate() < container->at(first)->evaluate()) { // and j is less than first, swap
//				first = j;
//			}
//		}
//		container->swap(first, i); // swap first and i, when done
//	}
//}
